    ...
    <v-card >
      <validation-observer ref="observer" v-slot="{ invalid }">
        <v-toolbar dense flat>
          <v-toolbar-title>{{ $t("DAD0010.lunchOrderInfo") }}</v-toolbar-title>
          <v-spacer></v-spacer>
        </v-toolbar>
        <v-card-text >
              <!-- 注文日-->
              ...
              :label="$t('DAD0010.deliveryLocation')"
      ...
      <v-card height="100%">
        <v-toolbar-title>{{ $t("DAD0010.menu") }}</v-toolbar-title>
          <v-card-text>
  ...
  methods: {
    ...
    async fetchlunchComboData () {
      try {
        ...
        const lunchQuery = {}
        lunchQuery.isActive = true
        // lunchQuery.codeType = 'LUNCHNAME'
        // lunchQuery.codeType = 'PRICE'
        const [lunchList] = await Promise.all([
          this.$axios.get('lunches', { params: lunchQuery })
        ])

        // 画面に結果をセット
        this.lunchList = lunchList.data
      } catch (error) {
        // エラーメッセージを表示
