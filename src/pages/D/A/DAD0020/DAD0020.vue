<template>
  <div class="d-flex flex-column flex-grow-1">
         <div >
            <div>
             <!--画面タイトル-->
             <div class="display-1">{{ $t("DAD0020.title") }}</div>
             <!--パンくずリスト-->
        ...
        <v-card>
          ...
          <v-snackbar v-model="snackbar" timeout="-1" right>
             <div>前月度：{{ previousSummary }} 円</div>
             <div>今月度：{{ currentSummary }} 円</div>
          </v-snackbar>
  ...
  methods: {
    ...
    async getEvents () {
      try {
        ...
        const response = await this.$axios.get('admin/lunchOrder', {
          params: query
        })

        // 確定金額を取得
        const money = await this.$axios.get(`admin/lunchOrder/${this.userId}`)
    ...
    // ユーザー選択されていたら編集画面に遷移する
    searchDay (item) {
      if (this.userId !== null) {
        this.$router.push({ name: 'DAD0020-detail', params: { date: item.date, userId: this.userId } })
      }
    }
  }
}
