<template>
    <div class="d-flex flex-column flex-grow-1">
        ...
        <v-card>
            ...
            <router-link :to="{ name: 'ZAD0070-detail', params: { companyCd: item.companyCd } }">
            {{ item.companyCd }}
            </router-link>
            </template>
            >
...
export default {
  ...
  computed: {
    ...
    headers () {
      return [
        { text: this.$t('ZAD0070.companyCd'), value: 'companyCd' },
        { text: this.$t('ZAD0070.companyName'), value: 'companyName' },
  ...
  methods: {
    ...
    async search () {
      try {
        ...
        // データ取得
        const response = await this.$axios.get('admin/associateCompanies', {
          params: query
        })
    ...
    // 新規登録画面に遷移する
    openNew () {
      this.$router.push({ name: 'ZAD0070-new' })
    }
  }
}
</script>
