<template>
  <div class="d-flex flex-column flex-grow-1">
    <div class="d-flex align-center">
      <div>
        <!-- 画面タイトル -->
        <div class="display-1">{{ $t("ZAD0010.title") }}</div>
        <!-- パンくずリスト -->
    ...
    <v-card>
      ...
      >
        ...
        <template v-slot:[`item.companyCd`]="{ item }">
          <router-link :to="{ name: 'ZAD0010-detail', params: { companyCd: item.companyCd } }">
            {{ item.companyCd }}
          </router-link>
        </template>
...
export default {
  ...
  computed: {
    ...
    headers () {
      return [
        { text: this.$t('ZAD0010.companyCd'), value: 'companyCd' },
        { text: this.$t('ZAD0010.companyName'), value: 'companyName' },
        { text: this.$t('ZAD0010.isActive'), value: 'isActive' }
      ]
    }
  },
  watch: {
  ...
  methods: {
    ...
    async search () {
      try {
        ...
        // データ取得
        const response = await this.$axios.get('/admin/companies', {
          params: query
        })
    ...
    // 新規登録画面に遷移する
    openNew () {
      this.$router.push({ name: 'ZAD0010-new' })
    }
  }
}
</script>
